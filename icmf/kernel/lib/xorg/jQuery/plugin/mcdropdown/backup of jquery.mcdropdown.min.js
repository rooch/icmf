(function($) {
	...
	var browser = {};
	browser.isIE = ($.browser.msie || (/\sTrident\/\d/i)
	...
	$.mcDropDownMenu = function(el, list, options) {
			...
			if (settings.mouseintent) {
				...
				settings.mouseintent = $
						.extend(
								{},
								mouseintentDefaults,
								$.isPlainObject(settings.mouseintent) ? settings.mouseintent
										: {})
			}
		}
		if (settings.click == null) {
			settings.click = function(e, dropdown, settings) {
				if (this.attr(settings.valueAttr)) {
					...
					dropdown.setValue($(this.parents("li")[0]).attr(
							settings.valueAttr))
				}
			}
		}
		...
		function getNodeText($el) {
			var nodeContent;
			var nContents = $el.contents().filter(
					function() {
		...
		function addBindings(el) {
			removeBindings(el);
			$("> li", el).bind("mouseover.mcdropdown", hoverOver).bind(
					"mouseout.mcdropdown", hoverOut)
		}
		function removeBindings(el) {
			$("> li", el).unbind(".mcdropdown")
		}
																					...
																					$
																							.extend(
																									...
																									settings.mouseintent,
																									{
																										mouseaway : function() {
																											if ($
																													.isFunction(settings.mouseintent.mouseaway)) {
																												settings.mouseintent.mouseaway
																														.apply(
																																this,
																																arguments)
																											}
		...
		function hoverOut(e) {
			...
			if ($child.length
					&& ((settings.mouseintent === false) || (!$child
							.data("mouseintent")))) {
			...
			var showShadow = function() {
				if (settings.dropShadow) {
					...
					$shadow.css({
						...
						right : pos.right + pos.marginRight,
						width : pos.width,
						height : pos.height,
						visibility : "visible"
					}).insertAfter($ul).show();
			...
			var pos = $.extend($el[bUseOffset === true ? "offset" : "position"]
					(),
					{
						...
						marginRight : parseInt(
								$.css($el[0], "marginRight", true), 10) || 0,
						marginRight : parseInt($.css($el[0], "marginRight",
								true), 10) || 0,
						marginTop : parseInt($.css($el[0], "marginTop", true),
								10) || 0,
						marginBottom : parseInt($.css($el[0], "marginBottom",
								true), 10) || 0
					});
			if (pos.marginTop < 0) {
				pos.top += pos.marginTop
			}
			if (pos.marginRight < 0) {
				pos.right += pos.marginRight
			}
			pos.bottom = pos.top + pos.height;
			pos.right = pos.right + pos.width;
			if (bHidden) {
		...
		function getPadding(el, name) {
			var torl = name == "height" ? "Top" : "Right", borr = name == "height" ? "Bottom"
					: "Left";
			return (parseInt("0" + $.css(el, "border" + torl + "Width", true),
					10)
					+ parseInt(
							"0" + $.css(el, "border" + borr + "Width", true),
							10)
					+ parseInt("0" + $.css(el, "padding" + torl, true), 10)
					+ parseInt("0" + $.css(el, "padding" + borr, true), 10)
					+ parseInt("0" + $.css(el, "margin" + torl, true), 10) + parseInt(
					"0" + $.css(el, "margin" + borr, true), 10))
		}
		...
		function columnizeList($el) {
			...
			$children
					.each(function(i) {
						...
						$li.css({
							marginRight : (widths.column * column),
							marginTop : marginTop,
							width : widths.item
		...
		function onBlur(e) {
			...
			iBlurTimeout = setTimeout(
					function() {
						var $current = getCurrentListItem();
						if (!settings.allowParentSelect
								&& $current.is(".mc_parent")) {
							var value = $current.find(
									"li:not('.mc_parent'):first").attr(
									settings.valueAttr);
							thismenu.setValue(value, true)
						}
						var results = thismenu.getValue(), value = results[0], name = results[1];
						$hidden.triggerHandler("update", [ value, name, this ]);
						$self.triggerHandler("update", [ value, name, this ]);
						if (settings.select != null) {
							settings.select.apply(thismenu, [ value, name ])
		...
		function hideMatches() {
			if (browser.isIE6 && !!$.fn.bgIframe && $keylistiframe) {
				$keylistiframe.hide()
			}
			if ($keylist) {
				$keylist.hide()
