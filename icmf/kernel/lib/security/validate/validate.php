...
//		print_r($_POST);
		...
		foreach($variables as $method=>$methodValue){
			if($this->debug){
				echo "<div class='whiteFrame paddy10 ltr'>";
			...
			if(count($methodValue) > 0){
				foreach($methodValue as $key=>$value){
					if(strstr($value, "::")){
						...
						if($this->debug){
							echo "<ul style='list-style:disc;'>";
							echo "<li>name:$key, value: $valueArr[0]</li>";
						}
						$validString = explode("<>", $valueArr[1]);

						switch ($method){
							case "GET":
								$_GET[$key] = mysql_real_escape_string($valueArr[0]);
								break;
							case "POST":
								$_POST[$key] = mysql_real_escape_string($valueArr[0]);
								break;
							case "COOKIE":
								$_COOKIE[$key] = mysql_real_escape_string($valueArr[0]);
								break;
							case "SERVER":
								$_SERVER[$key] = mysql_real_escape_string($valueArr[0]);
								break;
							case "SESSION":
								$_SESSION[$key] = mysql_real_escape_string($valueArr[0]);
								break;
						}
...
//					}
	...
	public function wordFilter($word){
		...
		$system->dbm->db->select("`word`", "`badWord`", "`word` LIKE '$word'");
