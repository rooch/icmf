...
//		echo "<div style='direction: ltr'>";
		if($info['id']){
			...
			$this->title = $info['title'] = $this->titleMaker('best');
			$this->description = $info['description'] = $this->metaDescriptionMaker('first');
			$this->robots = $info['robots'] = $this->robots();
			$this->keywords = $keywords = $this->metaKeywordMaker('silent');
			$this->author = $info['author'] = $this->author();
			$this->image = $info['image'] = $this->image();

			$permission = $system->dbm->db->informer("`access`", "`op` = '$_GET[op]' AND `mode` = '$_GET[mode]'");

			if($this->url && $permission['mur'] == 1 && $permission['mux'] == 1){
				if($mode == 'add' && $info['robots'] != 'NOINDEX, FOLLOW'){
...
//		echo "</div>";
	...
	public function metaDescriptionMaker($type='best'){
		...
		if(empty($this->url)){
			return mysql_real_escape_string($settings[description]);
		}else{
			if(!empty($_GET['p'])){
				return $lang['page'] . ' ' . $_GET['p'] . ' ' . $lang['from'] . ' ' . $lang[$_GET['op']];
			}else{
				return mysql_real_escape_string($this->p($type));
			}
		}
	}
	...
	public function titleMaker($type='best'){
		...
		if(empty($this->url)){
			...
			if($this->h1($type)){
				return $settings['siteName'] . ' - ' . mysql_real_escape_string($this->h1($type));
			}elseif($this->h2($type)){
				return $settings['siteName'] . ' - ' . mysql_real_escape_string($this->h2($type));
			}elseif($this->h3($type)){
				return $settings['siteName'] . ' - ' . mysql_real_escape_string($this->h3($type));
			}elseif($this->h4($type)){
				return $settings['siteName'] . ' - ' . mysql_real_escape_string($this->h4($type));
			}elseif($this->h5($type)){
				return $settings['siteName'] . ' - ' . mysql_real_escape_string($this->h5($type));
			}elseif($this->h6($type)){
				return $settings['siteName'] . ' - ' . mysql_real_escape_string($this->h6($type));
			}else{
				return $settings['siteName'] . ' - ' . mysql_real_escape_string($settings['websiteTitle']);
			}
		}
	}
	...
	public function img($type='first'){
		global $settings;
		...
		if ($tags->length) {
			...
			$values[] = $settings['domainName'] . '/' . $settings['theme'] . '/img/logo.png';
		}
		if(count($values) > 0)
		return $this->similar($values, $type);
	}
	...
	public function image($type='first'){
		...
		if($tag = $this->xpath->query('//img[@class="mainImage"]')){
			if ($tag->length) {
				$values[] = $tag->item(0)->getAttribute('src');
			}else{
				$values[] = $this->img('second');
			}
		}else{
			$values[] = $this->img('second');
		}
