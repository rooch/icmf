<?php
Define("organizationConfFile", "$settings[moduleAddress]/$sysVar[op]/config/config");
$config = organizationConfFile . ".php";
...
if(file_exists($config)){
	...
	if(file_exists($subSystem)){
		...
		$m_organization = new m_organization();
	...
	if(file_exists($subSystem)){
		require_once ($subSystem);
		$c_organization = new c_organization();
		$system->run($subSystem, 'On');
	}else{
		$system->run($subSystem, 'Off');
	}
	...
	die("Organization sub system is down !");
}
?>