...
//		$filter = $system->filterSplitter($filter);
		...
		$system->xorg->pagination->paginateStart("groupMan", "v_listMembers", "`id`, `active`, `timeStamp`, `userName`, `firstName`, `lastName`, `userPic`", "`$settings[userTable]`", "", "`timeStamp` DESC", "", "", "", "", 10, 7);
		$count = 1;
		while ($row = $system->dbm->db->fetch_array()){
